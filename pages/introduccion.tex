...
\cotutor{Este primer párrafo es muy confuso, y usa expresiones vagas como ``aparentemente" y ``miserablemente". Mi recomendación es que elimines el párrafo y comiences directamente con la descripción de lo que es la integración continua}

\remove{Actualmente, tareas aparentemente fundamentales como las pruebas, revisiones de código o compilaciones frecuentes de software deben ser difíciles de llevar a cabo debido a que la frecuencia de uso en proyectos software es miserablemente baja.
...
La integración continua\remove{o CI}\add{, o CI, como se conoce a menudo por su abreviatura en inglés,} es una práctica de desarrollo de software mediante la cual los desarrolladores combinan los cambios de código en un repositorio central de forma periódica, \remove{permitiendo la ejecución de versiones y la realización de pruebas automáticas sobre las mismas}\add{comprobándose que el código se comporta de la forma esperada a través de pruebas automáticas}. 
...
\begin{enumerate}
    ...
    \item Poco después de que se produzca una confirmación, el servidor de CI detecta que se han producido cambios en el repositorio de control de versiones, por lo que el servidor de CI recupera la última copia del código del repositorio y luego ejecuta un script de compilación, que integra el software. \cotutor{Aquí faltaría, almenos, comentar que se ejecutan los test. El script de compilación no ``integra" el software, simplemente comprueba que se puede construir.}
    \item \remove{El servidor de CI genera comentarios enviando por correo electrónico los resultados de la compilación a los miembros del proyecto que se hayan especificados.}\add{El servidor de CI captura los resultados de la compilación y ejecución de los test, enviándolos, por ejemplo, por correo electrónico a los miembros del equipo}
    \item El servidor de CI continúa buscando cambios en el depósito de control de versiones. \cotutor{Este último paso no es necesario comentarlo, se presupone que sigue haciendo polling al repositorio}
\end{enumerate}
...
\begin{compactitem}
    ...
    \item Tener un script de compilación ``build''. \cotutor{Para realizar integración continua no es necesario un script de compilación. Por ejemplo en Python no se compila el código, ¿significaría esto que no podemos realizar integración continua con python? Habría que especificar que este script construye y ejecuta los test del software que estamos desarrollando}
...
Una vez que se ejecuta una compilación o ``build'' automatizada con cada cambio en el sistema de control de versiones, se pueden agregar otras funciones o subprocesos al sistema de CI. Algunas funciones dependen de otras; por ejemplo, las pruebas automatizadas \cotutor{De nuevo, las pruebas automatizadas son parte fundamental del CI, no un añadido como su puede serlo, por ejemplo, el análisis estático} dependen de la compilación del código fuente. Este proceso repetitivo puede ayudar a reducir riesgos a lo largo del ciclo de vida del desarrollo.
...
\begin{compactitem}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.325\textwidth,clip=true]{\logoJenkins}
